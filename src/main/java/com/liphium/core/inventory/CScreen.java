...
public abstract class CScreen {
	...
	private final HashMap<Player, HashMap<Integer, CItem>> customItems =
		new HashMap<>();
	...
	public void setItemNotCached(Player player, int slot, CItem item) {
		HashMap<Integer, CItem> items = customItems.getOrDefault(
			player,
			new HashMap<>()
		);
		items.put(slot, item);
	...
	public void background() {
		ItemStack stack = new ItemStackBuilder(
			Material.BLACK_STAINED_GLASS_PANE
		)
			.withName(Component.empty())
			.buildStack();
		for (int i = 0; i < 9; i++) setItem(i, new CItem(stack).notClickable());
		for (int i = rows * 9 - 9; i < rows * 9; i++) setItem(
			i,
			new CItem(stack).notClickable()
		);
	}
	...
	public void background(Player player) {
		ItemStack stack = new ItemStackBuilder(
			...
			.withName(Component.empty())
			.buildStack();
		for (int i = 0; i < 9; i++) setItemNotCached(
			player,
			i,
			new CItem(stack).notClickable()
		);
		for (int i = rows * 9 - 9; i < rows * 9; i++) setItemNotCached(
			player,
			i,
			new CItem(stack).notClickable()
		);
	...
	public Inventory buildInventory(Player player) {
		...
		for (Map.Entry<Integer, CItem> entry : customItems
