...
public abstract class CScreen {
    ...
    public void background() {
        ItemStack stack = new ItemStackBuilder(Material.BLACK_STAINED_GLASS_PANE).withName(Component.empty()).buildStack();
        for (int i = 0; i < 9; i++) setItem(i, new CItem(stack).notClickable());
        for (int i = rows * 9 - 9; i < rows * 9; i++)
            setItem(i, new CItem(stack).notClickable());
    }

    public void background(Player player) {
        ItemStack stack = new ItemStackBuilder(Material.BLACK_STAINED_GLASS_PANE).withName(Component.empty()).buildStack();
        for (int i = 0; i < 9; i++)
            setItemNotCached(player, i, new CItem(stack).notClickable());
        for (int i = rows * 9 - 9; i < rows * 9; i++)
            setItemNotCached(player, i, new CItem(stack).notClickable());
    }
